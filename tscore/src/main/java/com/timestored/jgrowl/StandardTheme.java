...
class StandardTheme implements Theme {
	...
	@Override public int getLeftRuler(JFrame parentFrame) {
		return Math.max(0, parentFrame.getX() + parentFrame.getWidth()
	...
	/** {@inheritDoc} */
	@Override public JWindow getWindow(Growl message, final JFrame parentFrame) {
		...
		final JLabel label = new JLabel("<html><body style='width:" 
				+ (MESSAGE_WIDTH-BORDER_WIDTH-SPACE_HACK) + "px'>" 
