...
class FadingGrowler {
	...
	public synchronized void show(String message, String title, 
			Icon icon, boolean sticky, Level logLevel) {
	...
	private void addItem(final Growl message) {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			@Override public void run() {
				JWindow frame = theme.getWindow(message, parentFrame);
				synchronized (displayedItems) {
					int top = parentFrame.getY() + theme.getTopSpacer();
					if(displayedItems.size() > 0) {
						JWindow lastFrame = displayedItems.get(displayedItems.size() - 1).frame;
						top = lastFrame.getY() + lastFrame.getHeight()	+ theme.getSpaceBetweenItems();
					}
					frame.setLocation(theme.getLeftRuler(parentFrame), top);
					displayedItems.add(new DisplayedItem(frame, message));
				}
			}
		});
	}
	...
	private void updateDisplayItems() {
		int parentTop = parentFrame.getY() +  theme.getTopSpacer();
		int parentYmove = 0;
		// window moved
		...
		while (it.hasNext()) {
			...
			final JWindow fm = dispItem.frame;
			int y = fm.getY();
