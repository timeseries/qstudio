...
//		};
		...
		closeFolderAction = new AbstractAction(Msg.get(Key.CLOSE_FOLDER)) {
...
//        putValue(MNEMONIC_KEY, KeyEvent.VK_S);
		...
		saveFileAction = new ShortcutAction(Msg.get(Key.SAVE_FILE), 
				CIcon.SAVE, VK_S) {
	...
	public Action getCloseFileAction(final Document document) {
		...
		Action a = new AbstractAction(Msg.get(Key.CLOSE) + " " + t) {
			
			private static final long serialVersionUID = 1L;

			@Override public void actionPerformed(ActionEvent arg0) {
	            closeWithConfirmation(document);
		   }
		};
	...
	public Action openAllAction(final List<String> filePaths) {
		return new AbstractAction(Msg.get(Key.OPEN_ALL_RECENT)) {
			@Override public void actionPerformed(ActionEvent e) {
