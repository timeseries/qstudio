...
public class c {
	 ...
	 * //c.setEncoding
	 * ("UTF-8");O("Unicode "+c.k("{`$x}","Ranby Bj̦rklund AB".
	...
	/* ###### RYAN ####
	 * 2013-05-02 - Return names of native functions. 
	 */
	...
	static {
		final String[] monadics ={null,"+:","-:","*:","%:","&:","|:","^:","=:","<:",
			">:","$:",",:","#:","_:","~:","!:","?:","@:",".:","0::","1::","2::","avg",
			"last","sum","prd","min","max","exit","getenv","abs","sqrt","log","exp",
			"sin","asin","cos","acos","tan","atan", "enlist", "var", "dev", "hopen"};
		FUNC_1 = toChars(monadics);

		final String[] dyads ={":","+","-","*","%","&","|","^","=","<",">","$",
			",","#","_","~","!","?","@",".","0:","1:","2:","in","within","like","bin",
			"ss","insert","wsum","wavg","div","xexp","setenv", "binr", "cov", "cor"};
		FUNC_2 = toChars(dyads);
	}

	private static char[][] toChars(final String[] monadics) {
	...
	public static class Second implements Comparable<Second> {
		...
		public String toString() {
			return i == ni ? "" : new Minute(i / 60).toString() + ':'
					+ i2(i % 60);
		}
	...
	java.util.Date rz() {
		double f = rf();
		return new java.util.Date(Double.isNaN(f) ? nj : gl(k
				+ Math.round(8.64e7 * f)));
	}
	...
	Object r() throws UnsupportedEncodingException {
		...
		if (t > 99) {
			...
			// Ryan - custom handling to show where native functions appear
			// this may change but seems consistant between kdb versions
			if(t==102 || t==101) {
				int p = b[j];
			...
			// for projections staple the parts together. First item is function, remainder is args
			if(t==104) {
				String s = "";
				boolean first = true;
				for (n = ri(); i < n; i++) {
			...
			// added by ryan for empty arguments of a projection
			if(t==101 && b[j]==-1) {
				j++;
				return "";
			}
			
			// type 127 is a sorted keyed table e.g. `s#([a:0 1 2h] num:9 8 7h)
			if(t == 127) {
				return new Dict(r(), r());
			}
	...
	public static int n(Object x) throws UnsupportedEncodingException {
		return x instanceof Dict ? n(((Dict) x).x)
				: x instanceof Flip ? n(((Flip) x).y[0])
						: x instanceof char[] ? new String((char[]) x)
								.getBytes(e).length : Array.getLength(x);
	}
	...
	public Object k(String s, Object x, Object y) throws KException,
			IOException {
		Object[] a = { cs(s), x, y };
		return k(a);
	}

	public Object k(String s, Object x, Object y, Object z) throws KException,
			IOException {
		Object[] a = { cs(s), x, y, z };
		return k(a);
	}
	...
	public static Object[] NULL = { null, new Boolean(false), new UUID(0, 0),
			...
			new Integer(ni), new Long(nj), new Float(nf), new Double(nf),
			new Character(' '), "", new Timestamp(nj), new Month(ni),
			new Date(nj), new java.util.Date(nj), new Timespan(nj),
